...
func clientCertificateFn(imageDigest []byte) (func(*tls.CertificateRequestInfo) (*tls.Certificate, error), error) {
	return func(info *tls.CertificateRequestInfo) (*tls.Certificate, error) {
		...
		return mintIdentity(info.RATLSChallenge, imageDigest)
	}, nil
}
