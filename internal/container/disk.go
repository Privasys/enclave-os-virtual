...
// RequireDiskPresent fails a digest-pinned disk:// ref whose image disk is
// not attached, with an error that names the remedy.
//
// It replaces a fallback that re-pinned such a ref to the newest attached
// disk of the same family. "Newest" was the directory mtime, which the host
// controls, so the host could swap in an older attached build of the same
// app. That build was attested, but the platform had not asked for it.
// Refusing the load leaves the choice with the platform: the management
// service sees the failed deployment and redeploys the version it intends.
func RequireDiskPresent(ref string) error {
	if !IsDiskRef(ref) {
		return nil
	}
	...
	if _, err := os.Stat(filepath.Join(dir, "oci-layout")); err != nil {
		return fmt.Errorf("container: image disk for %s is not attached; redeploy from the platform", ref)
	}
	return nil
}
