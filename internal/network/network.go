...
const (
	...
	// fallbackNameserver answers DNS on every major cloud's metadata plane
	// (GCE, and the usual suspects respond on the link-local metadata IP).
	fallbackNameserver = "169.254.169.254"
)
...
// writeResolvConf publishes the container resolv.conf: systemd-resolved's
// real upstream list when available, else the cloud metadata resolver.
// Containers reach either through the bridge NAT.
func writeResolvConf() error {
	content, err := os.ReadFile(upstreamResolvConf)
	if err != nil || !strings.Contains(string(content), "nameserver") {
		content = []byte("# generated by enclave-os-virtual (no systemd-resolved upstreams)\nnameserver " + fallbackNameserver + "\n")
	}
	if err := os.MkdirAll("/run/containers-dns", 0o755); err != nil {
		return err
	}
	return os.WriteFile(ResolvConfPath, content, 0o644)
}
