...
// indicates that the kernel's TSM subsystem is loaded and TDX attestation is
// available.
...
// Provision checks for /sys/kernel/config/tsm/report and initialises the
// configfs-tsm quote provider.
func (a *TDXAttester) Provision(logger *zap.Logger) error {
	...
	a.quoteProvider = qp
	logger.Info("TDX attester provisioned", zap.String("tsm_report_path", tsmReportPath))
	return nil
}
